...
def gaussian_contour_plot(rgb_image, gaze_points, sigma=1.0, cam_dir='mid', contour_levels=3):
    ...
    contours = plt.contourf(x, y, composite_gaussian, levels=contour_levels, cmap='binary_r', alpha=0.7)
...
class SituationalAwarenessDataset(Dataset):
    ...
    def __getitem__(self, frame_num, id):
        ...
        for i in range(0, end_point):
            ...
            pts2d_mid, _, _ = self.get_gaze_point(focus_hit_pt_i, loc, rot)
            raw_gaze_data.append(pts2d_mid)
        gaze_heatmap = gaussian_contour_plot(np.array(rgb_image), raw_gaze_data, sigma=40.0, cam_dir='mid', contour_levels=3)

        validity = frame_filter(frame_num, self.awareness_df)

        return rgb_image, instance_seg_image, gaze_heatmap, label_mask_image, validity
    ...
    def get_gaze_point(self, focus_hit_pt, loc, rot):
        ...
        #focus_hit_pt = self.recording_data_dict[frame_num]["FocusInfo"]["HitPoint"]

...
class SituationalAwarenessDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # CHANGES FOR MIDDLE AND SIDES STARTS HERE
        # Current implementation is for combining the orientations as 1 input with many channels
        ...
        # 3 seperate inputs implementation
        if self.use_rgb:
            if self.middle_andsides:
                input_images_middle = (rgb_image, instance_seg_image, gaze_heatmap)
                input_images_left = (rgb_left_image, instance_seg_left_image, gaze_heatmap_left)
                input_images_right = (rgb_right_image, instance_seg_right_image, gaze_heatmap_right)
            else:
                input_images = (rgb_image, instance_seg_image, gaze_heatmap)
        else:
            if self.middle_andsides:
                input_images_middle = (instance_seg_image, gaze_heatmap)
                input_images_left = (instance_seg_left_image, gaze_heatmap_left)
                input_images_right = (instance_seg_right_image, gaze_heatmap_right)
            else:
                input_images = (instance_seg_image, gaze_heatmap)

        if self.middle_andsides:
            final_input_image = torch.cat(input_images_middle)
            final_input_image_left = torch.cat(input_images_left)
            final_input_image_right = torch.cat(input_images_right)

            final_label_mask_image = label_mask_image
            final_label_mask_image_left = label_mask_image_left
            final_label_mask_image_right = label_mask_image_right

            final_ignore_mask = ignore_mask
            final_ignore_mask_left = ignore_mask_left
            final_ignore_mask_right = ignore_mask_right

            padded_tensor = torch.nn.functional.pad(final_input_image, (0, 0, 4, 4), mode='constant', value=0)
            padded_tensor_left = torch.nn.functional.pad(final_input_image_left, (0, 0, 4, 4), mode='constant', value=0)
            padded_tensor_right = torch.nn.functional.pad(final_input_image_right, (0, 0, 4, 4), mode='constant', value=0)

            padded_label_mask_image_tensor = torch.nn.functional.pad(final_label_mask_image, (0, 0, 4, 4), mode='constant', value=0)
            padded_label_mask_image_tensor_left = torch.nn.functional.pad(final_label_mask_image_left, (0, 0, 4, 4), mode='constant', value=0)
            padded_label_mask_image_tensor_right = torch.nn.functional.pad(final_label_mask_image_right, (0, 0, 4, 4), mode='constant', value=0)

            padded_final_ignore_mask = torch.nn.functional.pad(final_ignore_mask.permute(2, 0, 1), (0, 0, 4, 4), mode='constant', value=0)
            padded_final_ignore_mask_left = torch.nn.functional.pad(final_ignore_mask_left.permute(2, 0, 1), (0, 0, 4, 4), mode='constant', value=0)
            padded_final_ignore_mask_right = torch.nn.functional.pad(final_ignore_mask_right.permute(2, 0, 1), (0, 0, 4, 4), mode='constant', value=0)

            return padded_tensor, padded_label_mask_image_tensor, padded_final_ignore_mask , \
                     padded_tensor_left, padded_label_mask_image_tensor_left, padded_final_ignore_mask_left, \
                        padded_tensor_right, padded_label_mask_image_tensor_right, padded_final_ignore_mask_right
        else:
            final_input_image = torch.cat(input_images)
            final_label_mask_image = label_mask_image
            final_ignore_mask = ignore_mask
